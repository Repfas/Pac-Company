...
from Pac_company_dagster.defs.asset.product import extract_product_raw
...
@asset_check(asset=extract_product_raw)
def qc_raw_data_product(context: OpExecutionContext, raw_data_path): 
    '''This function is a quality check of raw_data'''
    spark = SparkSession.builder.getOrCreate()
    context.log.info('Data quality checking for raw_data')
    ...
    for col in df.columns:
        null_count = df.filter(F.col(f"`{col}`").isNull()).count()
